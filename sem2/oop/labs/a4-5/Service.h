...
public:
	...
	//get all coats
	DynamicVector<TrenchCoat>& get_all_coats();

	//get all coats with a given size; if size is empty, get all coats
	DynamicVector<TrenchCoat> get_coats_by_size(const std::string& size);


	//get the basket
	DynamicVector<TrenchCoat> get_basket();
	...
	DynamicVector<TrenchCoat> filter_by_length(int maxLength);
};
